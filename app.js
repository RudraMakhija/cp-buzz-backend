const express = require("express"), axios = require("axios"), dotenv = require("dotenv");
...
const PORT = process.env.PORT || 3002, clistUrl = process.env.APP_CLIST_URL, apiKey = process.env.API_KEY;
  ...
  try {
    const promises = host_sites.map(async (site) => {
      const response = await axios.get(
        `${clistUrl}?resource=${site}&end__gt=${query.end__gt}&start__gt=${query.start__gt}&limit=150&format=json`,
        {
